...
public class OrderControllerTest extends WebMvcTestBase {
	...
	void 관리자_주문_전체조회_가능하다() throws Exception {
		...
		List<OrderResponseDto> orderList = List.of(
			new OrderResponseDto(1L, "uuid1", 1L, "dohan1", 1L, "chicken", 2500L, Status.ON_SALE.getStatus(),
				OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now()),
			new OrderResponseDto(2L, "uuid1", 3L, "dohan2", 2L, "pizza", 3500L, Status.ON_SALE.getStatus(),
				OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now())
		);
	...
	void 참여자_주문_목록_조회() throws Exception {
		...
		//테스트 전제 조건 및 환경 설정
		List<OrderResponseDto> orderList = List.of(
			new OrderResponseDto(1L, "uuid1", 3L, "dohan2", 1L, "chicken", 2500L, Status.ON_SALE.getStatus(),
				OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now()),
			new OrderResponseDto(2L, "uuid1", 3L, "dohan2", 2L, "pizza", 3500L, Status.ON_SALE.getStatus(),
				OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now())
		);
	...
	void 주문자_주문내역_삭제() throws Exception {
		// Given
		//테스트 전제 조건 및 환경 설정
		OrderResponseDto responseDto = new OrderResponseDto(1L, "uuid1", 3L, "dohan1", 1L, "chicken", 2500L,
			Status.ON_SALE.getStatus(), OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now());
	...
	void 관리자_주문_단건_조회() throws Exception {
		// Given
		//테스트 전제 조건 및 환경 설정
		OrderResponseDto responseDto = new OrderResponseDto(1L, "uuid1", 1L, "dohan1", 1L, "chicken", 2500L,
			Status.ON_SALE.getStatus(), OrderStatus.PENDING_PAYMENT.getOrderStatus(), LocalDateTime.now());
	...
	void 참여자_본인_주문_단건_조회() throws Exception {
		...
		//테스트 전제 조건 및 환경 설정
		OrderResponseDto responseDto = new OrderResponseDto(1L, "uuid1", 3L, "dohan1", 1L, "chicken", 2500L,
			Status.ON_SALE.getStatus(), OrderStatus.CONFIRMED.getOrderStatus(), LocalDateTime.now());
