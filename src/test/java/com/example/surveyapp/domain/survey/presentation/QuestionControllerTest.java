...
public class QuestionControllerTest extends WebMvcTestBase {
	...
	void 질문_단건조회한다() throws Exception {
		...
		ResultActions actions = mockMvc.perform(
			get("/api/survey/{surveyId}/question/{questionId}", surveyId, questionId)
				.header(HttpHeaders.AUTHORIZATION, "Bearer {jwt_token}"));
	...
	void 질문_상세정보를_수정한다() throws Exception {
		...
		when(questionService.updateQuestion(eq(userId), eq(surveyId), eq(questionId),
			any(QuestionUpdateRequestDto.class)))
			.thenReturn(responseDto);

		ResultActions actions = mockMvc.perform(
			patch("/api/survey/{surveyId}/question/{questionId}", surveyId, questionId)
				.header(HttpHeaders.AUTHORIZATION, "Bearer {jwt_token}")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(requestDto)));
	...
	void 질문을_삭제한다() throws Exception {
		...
		ResultActions actions = mockMvc.perform(
			delete("/api/survey/{surveyId}/question/{questionId}", surveyId, questionId)
				.header(HttpHeaders.AUTHORIZATION, "Bearer {jwt_token}"));
